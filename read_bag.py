...
def read_pcd_ros(msg):
    fields_name = get_fields_name(msg.fields)
    data_dict = {field:[] for field in fields_name}
    points_num = msg.width
    s_type, _ = type_trans_ros(msg.fields)
...
def get_ros_header_info(header, domain):
    return  {
...
def get_ros_pcd_field(msg, domain=''):
...
def get_ros_msg_all(msg, domain='', pass_domain=[]):
    attr_list = get_ros_msg_attr(msg)
    ret_dict = {}
    for attr in attr_list:
        ...
        if attr == 'header':
            new_ret_dict = get_ros_header_info(msg.header, new_domain)
            ret_dict.update(new_ret_dict.items())
            continue

        if type(getattr(msg, attr)).__name__.startswith('_'):
            new_ret_dict = get_ros_msg_all(getattr(msg, attr), new_domain, pass_domain)
            ret_dict.update(new_ret_dict.items())
        else:
    ...
    with rosbag.Bag(bag_path, 'r') as bag:
        for topic,msg,t in bag.read_messages():
            ...
            if msg_type == 'PointCloud2':
                data_dict = read_pcd_ros(msg)

                ret_dict = get_ros_msg_all(msg, topic+':', [topic+':/data', topic+':/fields'])
                new_ret_dict = get_ros_pcd_field(msg, topic+':/feilds')
                ret_dict.update(new_ret_dict.items())

                sub_path = save_path + change_topic_name(topic) + '/'
                ...
                ret_dict = get_ros_msg_all(msg, topic+':', [topic+':/data'])
                ...
                ret_dict = get_ros_msg_all(msg, topic+':')
                sub_path = save_path + change_topic_name(topic) + '/'
                os.makedirs(sub_path, exist_ok=True)
                np.save(sub_path + str(t)+'.npy', ret_dict)

bag_path = '1675837765-1675837835.bag'
